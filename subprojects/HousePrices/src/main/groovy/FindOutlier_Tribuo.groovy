...
def fieldProcessors = [
        bedrooms : new DoubleFieldProcessor('bedrooms'),
        bathrooms: new DoubleFieldProcessor('bathrooms'),
]
def rowProcessor = new RowProcessor<Label>(new FieldResponseProcessor('price', '0', new AnomalyFactory()), fieldProcessors)
def dataSource = new CSVDataSource<Label>(uri, rowProcessor, true)
